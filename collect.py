...
def nameofexcel(filepath):
    ...
    # file = msoffcrypto.OfficeFile(open(filepath,'rb'))  # 读取原文件
    # file.load_key(password='VelvetSweatshop')  # 填入设置保护时设置的密码, 默认密码为 'VelvetSweatshop'
    # file.decrypt(open(filepath,'wb'))  # 解密后保存为新文件, 新文件无保护，可直接使用 xlrd 读取内容
...
def collectExcel(dir, excelDir, baseexcel, targetDir, mode="str", pos=7, threshold=0.39, num=7):
