...
def findFirstNum(table):
    ...
    for i in range(rows):
        for j in range(columns):
            # 判断是否为数字
            if (type(table.cell_value(i, j)) == (float) or
                    type(table.cell_value(i, j)) == (int) or
                    table.cell_value(i,j).replace(".","").isdigit()): # 防小数
                # 判断是不是年份  # 它将年份读为float #可能会漏掉一些数据
                if float(table.cell_value(i, j)) >= 1949 and float(table.cell_value(i, j)) <= 2030:
                    continue
                else:
                    return i, j
...
def pifalingshou(dir):
    ...
    for file in xlslist:
        ...
        if ("批" in table.cell_value(0, 0)) and ("零" in table.cell_value(0, 0)):
            ...
            if len(lisfinal) == 1 and temp==1:
                for i in range(lisfinal[0] + 1,rows):
                    ws.write(i,0,"批发业" + str(table.cell_value(i,0)))
            if len(lisfinal) == 1 and temp==2:
                for i in range(lisfinal[0] + 1,rows):
                    ws.write(i,0,"零售业" + str(table.cell_value(i,0)))
            wb.save(file)
...
def zhusucanyin(dir):
    ...
    for file in xlslist:
        ...
        if ("住宿" in table.cell_value(0, 0)) and ("餐饮" in table.cell_value(0, 0)):
            ...
            lisfinal = []
            for i in range(1, rows):
                if "一" in str(table.cell_value(i, 0)):  #14-8表格单独修改，住宿没有（一）
                    lis1.append(i)
                if "二" in str(table.cell_value(i, 0)):
                    lis2.append(i)
            for i in lis1:
